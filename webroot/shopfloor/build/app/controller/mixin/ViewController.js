...
Ext.define('ShopFloor.controller.mixin.ViewController', {
     ...
     *
     * Beside calling Ext.create() this also fires the panelpainted event
     *
     * @param cls {String} see {@link Ext.create}
     * @param config {Object} see {@link Ext.create}
     * @return the new panel
     */
    createPanel: function(cls, config) {

        // create the panel
        var panel = Ext.create(cls, config);
        
        //get any associated models for this panel in order to fire the proper panel painted event
        if (panel._associatedModels && panel._associatedModels.length > 0) {
            var m = panel._associatedModels, eventName;
            for (var i=0;i<m.length;i++) {
                this.getApplication().fireEvent('panelpainted'+m[i].toLowerCase(), panel);
            }
        }
