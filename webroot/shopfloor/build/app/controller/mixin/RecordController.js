...
Ext.define('ShopFloor.controller.mixin.RecordController', {
    ...
    setupModel: function(config) {
        ...
        var eventName = 'panelpainted'+config.modelName.toLowerCase();
        // render data
        if(config.renderable) {
            eventListeners.push({ event: eventName, fn: this.onPanelPainted, scope: this });
        }
    ...
    onRecordLoadingFailed: function(config, result) {
        ...
        // if the viewport is currently disabled enable it first,
        // otherwise the alert is not clickable, see CORE-148
        if(this.enable) { // requires ViewController mixin
    ...
    onSaveData: function(panel, counter) {
        ...
        rec.save({
            failure: function() {
                // if the viewport is currently disabled enable it first,
                // otherwise the alert is not clickable
                if(this.enable) { // requires ViewController mixin
                    this.enable('Viewport');
                }
