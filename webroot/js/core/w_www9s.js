$(function() {
    ...
    WWww9s = {
		...
		init: function(){
            ...
            if(window.currentPageTitle !='WWww9s'){
				window.currentPageTitle = 'WWww9s';
		...
		,index: function(){
            ...
            var dataTable = $('#page-table').dataTable({
                ...
                "bProcessing": true,
                "bServerSide": true,
                "sAjaxSource": "/w_www9s/index.json",
                "sDom": 'CRTfrtip',
                "oColVis": {
                    "aiExclude": [2]
                },
                ...
                "fnCreatedRow": function(nRow, aData, iDataIndex){
                    $('td:eq(2)', nRow).html(
					 	'<i class="icon-edit" style="cursor:pointer" onclick="WWww9s.view('+aData[2]+')"></i>'+
						'<i class="icon-remove" style="margin-left:15px;cursor:pointer" onclick="WWww9s.remove('+aData[2]+')"></i>'
					);
                }
            });
        }
        ,add: function(){
            ...
            // change title
            $('#page-title').html('Add New W Www9');
            // clone template
        ...
        ,view: function(id){
            ...
            var me = this;
            this.clear();
            $.get('/w_www9s/view/'+id+'.json',function(record){
                 // change title
            	$('#page-title').html('Edit W Www9');
		...
		,remove: function(id, callback){
			...
			me.showMessage('Delete?', 'Are you sure you want to delete this W Www9.', function(btn){
				if(btn=='yes'){
					$.ajax({
						type: 'POST',
						url: '/w_www9s/'+action+'.json',
